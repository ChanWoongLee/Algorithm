	...
	static void solve() {
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				water now = q.poll();
				if (visit[now.color == 3 ? 0 : 1][now.r][now.c] == -1)
					continue;
				for (int move = 0; move < 4; move++) {
					...
					if ((now.color == 3 && visit[1][nextR][nextC] == now.cnt + 1)
						...
						res++;
						visit[1][nextR][nextC] = -1;
						visit[0][nextR][nextC] = -1;
						continue;
					} // 간자리가 상대의 cnt일때 꽃피우고 continue

					if (visit[0][nextR][nextC] != 0 || visit[1][nextR][nextC] != 0) // 이미 다녀간 자리일때
						continue;
